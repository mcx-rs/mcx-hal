...
macro_rules! scg_register {
    ...
    (@write $register:ident, $lambda:block) => {
        unsafe {
...
impl SOSC {
    ...
    pub fn apply_external_clock<const FREQ: u32>() -> Result<(), Error> {
        ...
        scg_register!(@modify sosccsr, |_, w| w.soscerr().disabled_or_no_error());
        scg_register!(@modify ldocsr, |_, w| w.ldoen().enabled());
        scg_register!(@modify sosccfg, |_, w| w.erefs().external());
        scg_register!(@modify sosccfg, |_, w| w.range().bits(CHECK::<FREQ>::RANGE));
